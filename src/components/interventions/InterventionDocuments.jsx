...
export default function InterventionDocuments({ incidentId, documents = [], canAdd = true }) {
  ...
  const createDocMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['documents', incidentId] });
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      await createDocMutation.mutateAsync({
        ...
        visible_client: newDoc.visible_client
      });
      ...
      await base44.entities.InterventionLog.create({
        ...
        commentaire: `Document ajouté: ${newDoc.nom_fichier}`,
        metadata: { type: newDoc.type_fichier }
      });
    } catch (error) {
    ...
    <Card className="border-2 border-indigo-200">
      <CardHeader>
        <div className="flex items-center justify-between">
          <CardTitle className="text-lg font-heading text-indigo-700 flex items-center gap-2">
      ...
      <CardContent>
        {documents.length === 0 ? (
          <p className="text-gray-500 text-center py-4">
            {lang === 'fr' ? 'Aucun document attaché' : 'No documents attached'}
          </p>
        ) : (
              ...
              >
                <div className="flex items-start gap-3 flex-1">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-medium text-[#0077A8] truncate">{doc.nom_fichier}</p>
                    {doc.description && (
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      ...
                      {doc.visible_client && (
                        <Badge className="bg-green-500 text-xs">Visible client</Badge>
                      )}
                      {doc.taille && (
        ...
        <Dialog open={showAddDialog} onOpenChange={setShowAddDialog}>
          <DialogContent className="max-w-lg">
            <DialogHeader>
              <DialogTitle className="font-heading text-xl text-indigo-700">
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-2">
                <input
                  type="checkbox"
                  checked={newDoc.visible_client}
                  onChange={(e) => setNewDoc({...newDoc, visible_client: e.target.checked})}
                  className="w-4 h-4"
                />
                <label className="text-sm">Visible par le client</label>
              </div>
