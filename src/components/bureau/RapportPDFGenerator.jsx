...
export default function RapportPDFGenerator({ rapport, lang = 'fr' }) {
  const generatePDF = () => {
    ...
    doc.text('RAPPORT D\'ACTIVITÉ', pageWidth / 2, y, { align: 'center' });
    y += 10;

    doc.setFontSize(12);
    doc.setTextColor(0, 0, 0);
    doc.text(`${rapport.metadata.periode_type} - ${rapport.metadata.portee}`, pageWidth / 2, y, { align: 'center' });
    y += 8;
    ...
    doc.setFontSize(14);
    doc.setTextColor(0, 119, 168);
    doc.text('E. AVIS & SATISFACTION', 14, y);
    y += 10;

    doc.setFontSize(10);
    doc.setTextColor(0, 0, 0);
    doc.text(`Avis interventions: ${rapport.sections.avis.synthese.interventions.count} avis`, 14, y);
    y += 6;
    doc.text(`Moyenne globale: ${rapport.sections.avis.synthese.interventions.moyenne_globale}/5`, 14, y);
    y += 10;

    doc.text(`Avis application: ${rapport.sections.avis.synthese.application.count} avis`, 14, y);
    y += 6;
    if (rapport.sections.avis.synthese.application.moyennes.facilite) {
      doc.text(`Facilité d'utilisation: ${rapport.sections.avis.synthese.application.moyennes.facilite}/5`, 14, y);
      y += 6;
      doc.text(`Suivi des interventions: ${rapport.sections.avis.synthese.application.moyennes.suivi}/5`, 14, y);
      y += 10;
    }

    // Section F - Synthèse Direction
    if (y > 220) {
      doc.addPage();
      y = 20;
    }
    ...
    doc.setFontSize(14);
    doc.setTextColor(102, 58, 237);
    doc.text('F. SYNTHÈSE DIRECTION', 14, y);
    y += 10;
    ...
    y += 8;
    doc.text(`Taux de satisfaction: ${rapport.sections.synthese.taux_satisfaction}/5`, 14, y);
  ...
  const { sections, metadata } = rapport;
  const totalInterventions = 
    sections.interventions.workItems.length + 
    sections.interventions.incidents.length;
    ...
    <Card className="border-2 border-green-300">
      ...
      <CardContent className="space-y-4">
        ...
        <div className="bg-purple-50 rounded-lg p-4 border border-purple-200">
          <div className="grid grid-cols-2 gap-3 text-sm">
            <div>
              ...
              <Badge className="ml-2 bg-purple-600">{metadata.periode_type}</Badge>
            </div>
            <div>
              <span className="font-bold text-purple-900">Portée:</span>
              <Badge className="ml-2 bg-blue-600">{metadata.portee}</Badge>
            </div>
            <div className="col-span-2">
              <span className="font-bold text-purple-900">Période:</span>
              <span className="ml-2">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="bg-purple-50 rounded-lg p-4 border-2 border-purple-200">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-xs font-bold text-purple-900">Satisfaction</span>
            </div>
            <p className="text-2xl font-bold text-purple-700">
              {sections.synthese.taux_satisfaction}/5
            </p>
          </div>
        </div>
        ...
        <div className="space-y-3">
          ...
          <div className="bg-gray-50 rounded-lg p-3 border">
            <h4 className="font-bold text-sm mb-2">⭐ Section E - Avis</h4>
            <p className="text-xs text-gray-600">
              {sections.avis.synthese.interventions.count} avis interventions, {sections.avis.synthese.application.count} avis app
            </p>
          </div>
        </div>
      </CardContent>
