...
import { Send, Loader2, Smile, Meh, Frown, AlertTriangle, Download, Home, CheckCircle } from "lucide-react";
import { toast } from "sonner";

/* ============================================================
   COTTAGE PREMIUM - INVENTAIRE COMPLET
============================================================ */
...
const INVENTAIRE_COTTAGE = {
  ...
  couverts: [
    ...
    { id: 'plat_four', emoji: '🍛', label: 'Plat four rond', qty: 1, service: 'MENAGE' }
  ],
  batterie: [
...
export default function ClientControleInventaireCottage() {
  ...
  const cottage = sessionStorage.getItem("arrivee_numero");
  ...
  useEffect(() => {
    if (!nom || !prenom || !cottage || !dateArrivee || !dateDepart) {
  ...
  const createInterventionCottage = async ({ service, items, ficheId }) => {
    if (!items || items.length === 0) return;

    const hasUrgent = items.some(i => i.urgent);
    const description = items.map(i => `${i.label} (manquant: ${i.qtyManquante})`).join(', ');

    const intervention = await base44.entities.Intervention.create({
      sejour_id: `ARR-${cottage}-${dateArrivee.replace(/-/g, '')}-${Math.random().toString(36).substring(2, 8)}`,
      date_arrivee: dateArrivee,
      date_depart: dateDepart,
      client_nom: nom,
      client_prenom: prenom,
      logement_numero: cottage,
      categorie_logement: "Cottage Premium",
      contexte: "ARRIVEE",
      urgent: hasUrgent,
    ...
    await base44.entities.Notification.create({
      ...
      titre: `${hasUrgent ? '🔴 URGENT - ' : ''}Intervention ${service} - Cottage ${cottage}`,
  ...
  const handleFinalSubmit = async () => {
    ...
    try {
      ...
      await createInterventionCottage({ service: "TECHNIQUE", items: technique, ficheId: fiche.id });
  ...
  return (
    <div className="min-h-screen max-w-4xl mx-auto px-6 py-8">
      ...
      <p className="text-center text-gray-600 mb-6">
        Cottage Premium {cottage} • {nom} {prenom}
      </p>

      {Object.entries(INVENTAIRE_COTTAGE).map(([section, items]) => (
        <Card key={section} className="mb-6">
          <CardContent className="p-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {items.map(item => (
                <ItemRow key={item.id} item={item} section={section} />
              ))}
            </div>
          </CardContent>
        </Card>
