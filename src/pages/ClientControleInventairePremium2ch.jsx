...
/* ============================================================
   MH PREMIUM 2CH - INVENTAIRE COMPLET (R01 à R18 hors Twins)
============================================================ */

const INVENTAIRE_PREMIUM_2CH = {
  vaisselle: [
...
export default function ClientControleInventairePremium2ch() {
  ...
  const allItems = useMemo(() => {
    return [
      ...INVENTAIRE_PREMIUM_2CH.vaisselle,
  ...
  const createInterventionPremium2ch = async ({ service, items, ficheId }) => {
    ...
    const intervention = await base44.entities.Intervention.create({
      ...
      logement_numero: mh,
      categorie_logement: "MH Premium 2 chambres",
      contexte: "ARRIVEE",
      urgent: hasUrgent,
  ...
  const handleFinalSubmit = async () => {
    ...
    try {
      ...
      await createInterventionPremium2ch({ service: "MENAGE", items: menage, ficheId: fiche.id });
      await createInterventionPremium2ch({ service: "TECHNIQUE", items: technique, ficheId: fiche.id });
  ...
  return (
    <div className="min-h-screen max-w-4xl mx-auto px-6 py-8">
      ...
      <p className="text-center text-gray-600 mb-6">
        MH Premium 2ch {mh} • {nom} {prenom}
      </p>

      {Object.entries(INVENTAIRE_PREMIUM_2CH).map(([section, items]) => (
