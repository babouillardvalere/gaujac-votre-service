...
/* ============================================================
   CHALET CLASSIQUE 1CH - INVENTAIRE COMPLET (A01 à A04)
============================================================ */

const INVENTAIRE_CHALET_CLASSIQUE_1CH = {
  vaisselle: [
    { id: 'assiettes_creuses', emoji: '🍽️', label: 'Assiettes creuses', qty: 4, service: 'MENAGE' },
    { id: 'assiettes_dessert', emoji: '🍰', label: 'Assiettes à dessert', qty: 4, service: 'MENAGE' },
    ...
    { id: 'verres', emoji: '🥛', label: 'Verres', qty: 4, service: 'MENAGE' },
  ...
  electromenager: [
    ...
    { id: 'poele', emoji: '🍳', label: 'Poêle', qty: 1, service: 'MENAGE' },
  ...
  menage: [
    ...
    { id: 'poubelle', emoji: '🗑️', label: 'Poubelle', qty: 1, service: 'MENAGE' },
  ...
  chambre: [
    ...
    { id: 'cintres', emoji: '👗', label: 'Cintres', qty: 6, service: 'MENAGE' }
  ],
  sanitaires: [
...
export default function ClientControleInventaireChaletClassique1ch() {
  ...
  const allItems = useMemo(() => {
    return [
      ...INVENTAIRE_CHALET_CLASSIQUE_1CH.vaisselle,
      ...INVENTAIRE_CHALET_CLASSIQUE_1CH.couverts,
      ...INVENTAIRE_CHALET_CLASSIQUE_1CH.ustensiles,
      ...INVENTAIRE_CHALET_CLASSIQUE_1CH.electromenager,
      ...INVENTAIRE_CHALET_CLASSIQUE_1CH.menage,
      ...INVENTAIRE_CHALET_CLASSIQUE_1CH.exterieur,
      ...INVENTAIRE_CHALET_CLASSIQUE_1CH.chambre,
      ...INVENTAIRE_CHALET_CLASSIQUE_1CH.sanitaires
    ];
  }, []);
  ...
  const createInterventionChaletClassique = async ({ service, items, ficheId }) => {
    ...
    const intervention = await base44.entities.Intervention.create({
      ...
      logement_numero: mh,
      categorie_logement: "Chalet Classique 1 chambre",
      contexte: "ARRIVEE",
      urgent: hasUrgent,
    ...
    await base44.entities.Notification.create({
      ...
      titre: `${hasUrgent ? '🔴 URGENT - ' : ''}Intervention ${service} - Chalet ${mh}`,
  ...
  const handleFinalSubmit = async () => {
    ...
    try {
      ...
      await createInterventionChaletClassique({ service: "MENAGE", items: menage, ficheId: fiche.id });
      await createInterventionChaletClassique({ service: "TECHNIQUE", items: technique, ficheId: fiche.id });
      ...
      <div className="min-h-screen flex items-center justify-center px-4">
        <div className="text-center max-w-md space-y-6">
          ...
          <h2 className="text-2xl font-bold text-[#0077A8]">
            {lang === "fr" 
              ? `Nous vous remercions pour le retour de votre hébergement et nous vous souhaitons un excellent séjour au Camping Paradis Domaine de Gaujac ${prenom} ${nom}.`
              : `Thank you for your feedback and we wish you an excellent stay at Camping Paradis Domaine de Gaujac ${prenom} ${nom}.`
            }
          </h2>
  ...
  return (
    <div className="min-h-screen max-w-4xl mx-auto px-6 py-8">
      ...
      <p className="text-center text-gray-600 mb-6">
        Chalet Classique 1ch {mh} • {nom} {prenom}
      </p>

      {Object.entries(INVENTAIRE_CHALET_CLASSIQUE_1CH).map(([section, items]) => (
        <Card key={section} className="mb-6">
          <CardContent className="p-4">
            <h3 className="font-semibold text-[#0077A8] mb-3 capitalize">
              {lang === "fr" 
                ? section === 'vaisselle' ? '🍽️ Vaisselle & Cuisine' 
                  : section === 'couverts' ? '🍴 Couverts'
                  : section === 'ustensiles' ? '🍳 Ustensiles & Accessoires'
                  : section === 'electromenager' ? '🔌 Électroménager'
                  : section === 'menage' ? '🧹 Ménage & Sécurité'
                  : section === 'exterieur' ? '🌿 Extérieur / Accès'
                  : section === 'chambre' ? '🛏️ Chambre'
      ...
      <Card className="mb-6">
        <CardContent className="p-4">
          <h3 className="font-semibold text-[#0077A8] mb-3">
            {lang === "fr" ? "😊 Appréciation globale *" : "😊 Overall rating *"}
          </h3>
          <div className="flex gap-4 justify-center">
            <Button
              variant={noteGlobale === "insatisfaisant" ? "default" : "outline"}
              onClick={() => setNoteGlobale("insatisfaisant")}
              className={noteGlobale === "insatisfaisant" ? "bg-red-500" : ""}
            >
              <Frown className="w-5 h-5 mr-2" />
              {lang === "fr" ? "😠 Insatisfaisant" : "😠 Unsatisfactory"}
            </Button>
            <Button
              variant={noteGlobale === "correct" ? "default" : "outline"}
              onClick={() => setNoteGlobale("correct")}
              className={noteGlobale === "correct" ? "bg-gray-500" : ""}
            >
              <Meh className="w-5 h-5 mr-2" />
              {lang === "fr" ? "😐 Correct" : "😐 Correct"}
            </Button>
            <Button
              variant={noteGlobale === "tres_satisfaisant" ? "default" : "outline"}
              onClick={() => setNoteGlobale("tres_satisfaisant")}
              className={noteGlobale === "tres_satisfaisant" ? "bg-green-500" : ""}
            >
              <Smile className="w-5 h-5 mr-2" />
              {lang === "fr" ? "😄 Très satisfaisant" : "😄 Very satisfactory"}
            </Button>
          </div>
      ...
      <Button onClick={handlePrepareSubmit} className="w-full h-14 bg-[#00AEEF] mt-6" disabled={submitting}>
        <Send className="mr-2" />
        {lang === "fr" ? "Valider le contrôle inventaire" : "Confirm"}
      </Button>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-gray-50 p-4 rounded-lg">
              <p><strong>{lang === "fr" ? "Appréciation globale" : "Overall rating"}:</strong> {
                noteGlobale === "insatisfaisant" ? "😠 Insatisfaisant" :
                noteGlobale === "correct" ? "😐 Correct" :
                noteGlobale === "tres_satisfaisant" ? "😄 Très satisfaisant" : ""
              }</p>
