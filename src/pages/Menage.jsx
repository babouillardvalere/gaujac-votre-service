    ...
    if (incident.isGrouped && incident.workItems?.length > 0) {
      ...
      try {
        ...
        for (const wi of incident.workItems) {
          const tempsPriseEnCharge = wi.date_saisie ? differenceInMinutes(now, new Date(wi.date_saisie)) : 0;
          ...
          await base44.entities.InterventionLog.create({
            ...
            commentaire: 'Intervention prise en charge (groupe)'
          });
        ...
        await notifyBureau(`${incident.workItems.length} interventions ménage prises en charge par ${collaborateurNom} - ${incident.logement}`);
    ...
    try {
      ...
      for (const wi of missionGroup.workItems) {
        ...
        await pushClientEvent({
          incident: wi,
          type: 'TERMINEE',
          message: "Intervention ménage clôturée."
        });
      }
      ...
      const interventionsDetail = missionGroup.workItems.map(wi => {
        ...
        const catInfo = getCategoryInfo(tache?.objet_id || wi.categorie || 'materiel_menage');
        return {
          workitem_id: wi.workItemId,
          categorie: tache?.objet_id || wi.categorie || 'materiel_menage',
      ...
      await notifyBureau(`Mission ménage clôturée - ${missionGroup.logement} - ${missionGroup.workItems.length} intervention(s)`);
      
      await base44.entities.HistoriqueEvent.create({
        type_event: 'INTERVENTION_CLOTUREE',
        titre: `Mission ménage clôturée - ${missionGroup.logement}`,
                  ...
                  >
                    <CardContent className="p-4">
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <div className="flex items-center gap-2 flex-wrap">
                              ...
                              {isGrouped && (
                                <Badge className="bg-[#FFD700] text-[#0077A8] text-xs">
                      ...
                      {isGrouped ? (
                        ...
                        <div className="mb-3">
                          {(() => {
                            console.log('[MENAGE_RENDER] Incident', incident.logement, 'desc_op:', incident.description_operationnelle?.substring(0, 50), 'taches:', incident.taches?.length);
                            return null;
                          })()}
                ...
                <>
                  <div className="bg-[#FFF4B2] rounded-xl p-4 space-y-2">
                    ...
                    <div className="flex justify-between">
                      ...
                      <Badge className="bg-[#FFD700] text-[#0077A8]">{selectedIncident.workItems.length}</Badge>
                    </div>
                  </div>
                      ...
                      {selectedIncident.workItems.map((wi, idx) => {
                        ...
                        return (
                          <div key={idx} className="bg-yellow-50 border-l-4 border-yellow-500 p-3 rounded">
                            <p className="text-xs text-yellow-600 font-semibold mb-2">Intervention {idx + 1}:</p>
