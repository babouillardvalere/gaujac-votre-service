...
/* ============================================================
   MENAGE – COMPLET + SYNC SUIVI CLIENT (ARRIVEE)
============================================================ */
...
const isPhotoRequired = () => false; // ménage = recommandé, pas obligatoire
...
export default function Menage() {
  ...
  /* =======================
     SYNC CLIENT HELPERS
     - InterventionEvent (visible client)
     - SuiviInventaire.statut_menage
  ======================= */

  const mapIncidentStatusToSuivi = (incidentStatus) => {
    if (incidentStatus === "en_cours") return "en_cours";
    if (incidentStatus === "resolu") return "resolu";
    if (incidentStatus === "en_attente_materiel") return "en_attente";
    return "en_attente";
  };

  const updateSuiviInventaireStatus = async (incident, nextIncidentStatus) => {
    const ficheId = incident?.fiche_arrivee_id;
    if (!ficheId) return;

    const suivis = await base44.entities.SuiviInventaire.filter({ fiche_arrivee_id: ficheId }, "-created_at", 5);
    const suivi = suivis?.[0];
    if (!suivi?.id) return;

    await base44.entities.SuiviInventaire.update(suivi.id, {
      statut_menage: mapIncidentStatusToSuivi(nextIncidentStatus),
    });
  };

  const pushClientEvent = async ({ incident, type, message, attenteRaison = null, delaiEstime = null }) => {
    // IMPORTANT: ClientSuiviDetail lit InterventionEvent via intervention_id
    const interventionId = incident?.intervention_id;
    if (!interventionId) return;

    await base44.entities.InterventionEvent.create({
      intervention_id: interventionId,
      fiche_arrivee_id: incident.fiche_arrivee_id,
      type, // PRISE_EN_CHARGE / EN_COURS / EN_ATTENTE / TERMINEE (comme ClientSuiviDetail)
  ...
  const updateTacheMutation = useMutation({
    ...
    onSuccess: async ({ id, data, ancienStatut }) => {
      ...
      if (ancienStatut && data?.statut) {
        ...
        if (tache?.[0]) await notifierChangementStatutTache(tache[0], ancienStatut);
      }
    },
  });
  ...
  const handlePrendreEnCharge = (incident) => {
    ...
    // ménage: photo recommandée -> on laisse le choix via bouton "avec photo"
  ...
  const handlePrendreEnChargeSansPhoto = async (incident) => {
    ...
    // SYNC client
    await updateSuiviInventaireStatus(incident, "en_cours");
    await pushClientEvent({
      incident,
      type: "EN_COURS",
      message: lang === "fr" ? "L’équipe ménage est en cours d’intervention." : "Housekeeping is working on your request.",
    });

    // notif interne existante
  ...
  const handlePhotoAvantUploaded = async (photoData) => {
    ...
    // SYNC client
    await updateSuiviInventaireStatus(incidentForPhoto, "en_cours");
    await pushClientEvent({
    ...
    await notifierClientPriseEnCharge(incidentForPhoto, collaborateurNom);

    setIncidentForPhoto(null);
  };

  const handleTerminer = (incident) => {
    // ménage: photo recommandée -> on laisse choix "terminer avec photo"
  ...
  const handleTerminerSansPhoto = async (incident) => {
    ...
    // SYNC client
    await updateSuiviInventaireStatus(incident, "resolu");
    await pushClientEvent({
      incident,
      type: "TERMINEE",
      message: lang === "fr" ? "L’intervention ménage est terminée." : "Housekeeping intervention is completed.",
    });
  ...
  const handlePhotoApresUploaded = async (photoData) => {
    ...
    // SYNC client
    await updateSuiviInventaireStatus(incidentForPhoto, "resolu");
    await pushClientEvent({
      incident: incidentForPhoto,
      type: "TERMINEE",
      message: lang === "fr" ? "L’intervention ménage est terminée." : "Housekeeping intervention is completed.",
    ...
    setIncidentForPhoto(null);
    setCommentaire("");
    await notifierClientResolution(incidentForPhoto);
  };
  ...
  const confirmMettreEnAttente = async (formData) => {
    ...
    // SYNC client
    await updateSuiviInventaireStatus(incidentToWait, "en_attente_materiel");
    await pushClientEvent({
      incident: incidentToWait,
      type: "EN_ATTENTE",
      message: lang === "fr" ? "Intervention ménage en attente." : "Housekeeping is on hold.",
      attenteRaison: formData.raison || null,
      delaiEstime: formData.delai || null,
    });
  ...
  const sortByPriority = (a, b) => {
    ...
    if (typeA !== "urgent" && typeB === "urgent") return 1;
    if (typeA === "urgent" && typeB === "urgent") return new Date(a.date_saisie) - new Date(b.date_saisie);
      ...
      <div className="bg-[#FFD700] text-[#0077A8] px-4 py-4 sticky top-0 z-10">
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            <h2 className="font-heading text-xl">{t("menu_menage")}</h2>
          </div>
      ...
      <div className="max-w-4xl mx-auto px-4 py-6">
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <TabsList className="grid w-full grid-cols-3 mb-6">
            ...
            <TabsTrigger value="missions">{lang === "fr" ? "Missions" : "Missions"}</TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="interventions">
            ...
            <Tabs value={filter} onValueChange={setFilter} className="mb-6 mt-6">
              <TabsList className="bg-[#FFF4B2] p-1 rounded-xl border border-[#FFD700]/50 w-full grid grid-cols-4">
                <TabsTrigger
                  value="en_attente"
                  className="rounded-lg font-heading text-xs data-[state=active]:bg-[#FFA500] data-[state=active]:text-white relative"
                >
                  {t("en_attente")}
                ...
                <TabsTrigger
                  value="en_cours"
                  className="rounded-lg font-heading text-xs data-[state=active]:bg-[#FFD700] data-[state=active]:text-[#0077A8] relative"
                >
                  {t("en_cours")}
                  <span className="ml-1 min-w-5 h-5 inline-flex items-center justify-center text-xs font-bold rounded-full px-1.5 bg-[#E63946] text-white">
                ...
                <TabsTrigger
                  value="en_attente_materiel"
                  className="rounded-lg font-heading text-xs data-[state=active]:bg-gray-500 data-[state=active]:text-white relative"
                >
                  ⏳ {t("menu_attente")}
                  <span className="ml-1 min-w-5 h-5 inline-flex items-center justify-center text-xs font-bold rounded-full px-1.5 bg-[#E63946] text-white">
                ...
                <TabsTrigger
                  value="resolu"
                  className="rounded-lg font-heading text-xs data-[state=active]:bg-green-500 data-[state=active]:text-white relative"
                >
                  {t("resolu")}
                  <span className="ml-1 min-w-5 h-5 inline-flex items-center justify-center text-xs font-bold rounded-full px-1.5 bg-[#E63946] text-white">
                      ...
                      >
                        <CardContent className="p-4">
                          ...
                          {incident.autorisation_acces && (
                            <div
                              className={`mt-2 inline-flex items-center gap-1 px-2 py-1 rounded text-xs font-body ${
                          ...
                          {incident.pris_par && incident.statut === "en_cours" && incident.date_debut && (
                            <div className="mt-2 pt-2 border-t border-gray-100 flex items-center justify-between">
                              <p className="text-xs font-body text-[#FFD700]">{t("pris_en_charge_par")}: {incident.pris_par}</p>
                              <InterventionTimer startTime={incident.date_debut} isActive />
                            </div>
                          )}
            ...
            <Card className="border-2 border-[#FFD700]/30 rounded-xl mb-4">
              <CardContent className="p-4">
                <div className="grid grid-cols-3 gap-3">
                  <div>
                    <label className="text-xs font-heading text-[#0077A8] mb-1 block">{lang === "fr" ? "Statut" : "Status"}</label>
                    <select
                  ...
                  <div>
                    <label className="text-xs font-heading text-[#0077A8] mb-1 block">{lang === "fr" ? "Priorité" : "Priority"}</label>
                    <select
                  ...
                  <div>
                    <label className="text-xs font-heading text-[#0077A8] mb-1 block">{lang === "fr" ? "Trier par" : "Sort by"}</label>
                    <select
                ...
                const filteredTaches = taches
                  ...
                  .sort((a, b) => {
                    if (tacheFilters.tri === "priorite") {
                      ...
                      return (order[b.priorite] || 2) - (order[a.priorite] || 2);
                    }
                    if (tacheFilters.tri === "creation") return new Date(b.created_date) - new Date(a.created_date);
                ...
                return filteredTaches.map((tache) => (
                  <Card key={tache.id} className="border-2 border-[#FFD700]/30 rounded-xl">
                    <CardContent className="p-4">
                      ...
                      {tache.description && <p className="text-sm text-gray-600 whitespace-pre-line mb-3">{tache.description}</p>}
                ...
                <div className="space-y-3 pt-4 border-t">
                  ...
                  <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-3">
                    ...
                    <p className="text-xs text-yellow-600 font-body">
                      {lang === "fr"
                        ? "Facultatives pour le ménage. Recommandées pour attester de votre passage."
                        : "Optional for housekeeping. Recommended to prove your work."}
                    </p>
                  </div>
                  ...
                  >
                    ...
                    {lang === "fr" ? "Prendre en charge avec photo avant (recommandé)" : "Start with before photo (recommended)"}
                  </Button>
                </div>
              )}
                ...
                <div className="space-y-3 pt-4 border-t">
                  <div className="flex items-center justify-between">
                    <p className="text-sm font-body text-[#FFD700]">
                      {t("pris_en_charge_par")}: {selectedIncident.pris_par}
                    </p>
                    {selectedIncident.date_debut && <InterventionTimer startTime={selectedIncident.date_debut} isActive />}
                  </div>
                  ...
                  >
                    <Camera className="w-4 h-4 mr-2" />
                    {lang === "fr" ? "Terminer avec photo après (recommandé)" : "Finish with after photo (recommended)"}
                  </Button>
                </div>
              )}
                ...
                <div className="space-y-3 pt-4 border-t">
                  <div className="bg-[#FFA500]/10 p-4 rounded-xl border border-[#FFA500]/30">
                    ...
                    <p className="font-body text-gray-700">
                      <strong>{t("motif_label")} :</strong> {selectedIncident.motif_attente}
                    </p>
                    {selectedIncident.attente_materiel_detail && (
                      <p className="font-body text-gray-600 text-sm mt-1">
                        {t("materiel_necessaire")} : {selectedIncident.attente_materiel_detail}
                      </p>
                    )}
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <Button
                      onClick={async () => {
                        updateMutation.mutate({ id: selectedIncident.id, data: { statut: "en_cours" } });

                        await updateSuiviInventaireStatus(selectedIncident, "en_cours");
                        await pushClientEvent({
                          incident: selectedIncident,
                          type: "EN_COURS",
                          message: lang === "fr" ? "L’équipe ménage a repris l’intervention." : "Housekeeping resumed the intervention.",
                        });
                      }}
              ...
              {selectedIncident.statut === "resolu" && (
                <div className="bg-green-50 p-3 rounded-xl">
                  <p className="text-sm font-body text-green-700">
                    {t("resolu")} - {selectedIncident.pris_par} -{" "}
                    {selectedIncident.date_resolution && format(new Date(selectedIncident.date_resolution), "dd/MM/yyyy HH:mm")}
                  </p>
                </div>
              )}
            </div>
