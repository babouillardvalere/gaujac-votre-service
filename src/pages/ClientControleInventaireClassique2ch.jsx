...
/* ============================================================
   MH CLASSIQUE 2CH - INVENTAIRE COMPLET (D01 à D21)
============================================================ */

const INVENTAIRE_CLASSIQUE_2CH = {
  vaisselle: [
  ...
  appareils: [
    ...
    { id: 'congelateur', emoji: '🧊', label: 'Congélateur', qty: 1, service: 'TECHNIQUE' },
...
export default function ClientControleInventaireClassique2ch() {
  ...
  const allItems = useMemo(() => {
    return [
      ...INVENTAIRE_CLASSIQUE_2CH.vaisselle,
  ...
  const createInterventionClassique2ch = async ({ service, items, ficheId }) => {
    ...
    const intervention = await base44.entities.Intervention.create({
      ...
      logement_numero: mh,
      categorie_logement: "MH Classique 2 chambres",
      contexte: "ARRIVEE",
      urgent: hasUrgent,
  ...
  const handleFinalSubmit = async () => {
    ...
    try {
      ...
      await createInterventionClassique2ch({ service: "TECHNIQUE", items: technique, ficheId: fiche.id });
      ...
      <div className="min-h-screen flex items-center justify-center px-4">
        <div className="text-center max-w-md space-y-6">
          ...
          <h2 className="text-2xl font-bold text-[#0077A8]">
            {lang === "fr" 
              ? `Nous vous remercions pour le retour concernant votre hébergement. Nous vous souhaitons un excellent séjour parmi nous ${nom} ${prenom}.`
              : `Thank you for your feedback. We wish you an excellent stay with us ${nom} ${prenom}.`
            }
          </h2>
  ...
  return (
    <div className="min-h-screen max-w-4xl mx-auto px-6 py-8">
      ...
      <p className="text-center text-gray-600 mb-6">
        MH Classique 2ch {mh} • {nom} {prenom}
      </p>

      {Object.entries(INVENTAIRE_CLASSIQUE_2CH).map(([section, items]) => (
            ...
            <h3 className="font-semibold text-[#0077A8] mb-3 capitalize">
              {lang === "fr" 
                ? section === 'vaisselle' ? '🍽️ Vaisselle' 
                  ...
                  : section === 'appareils' ? '🔌 Cuisson / Électroménager'
                  : section === 'sanitaires' ? '🚿 Salle d\'eau'
                  : section === 'menage' ? '🧹 Équipements Ménage'
                  : section === 'literie' ? '🛏️ Chambres'
                  : section === 'exterieur' ? '🔑 Accès / Extérieur'
                  : section
                : section
              }
            </h3>
