...
/* ============================================================
   MH CLASSIQUE 3CH - INVENTAIRE COMPLET (L01 à L06)
============================================================ */

const INVENTAIRE_CLASSIQUE_3CH = {
  vaisselle: [
...
export default function ClientControleInventaireClassique3ch() {
  ...
  const allItems = useMemo(() => {
    return [
      ...INVENTAIRE_CLASSIQUE_3CH.vaisselle,
  ...
  const createInterventionClassique3ch = async ({ service, items, ficheId }) => {
    ...
    const intervention = await base44.entities.Intervention.create({
      ...
      logement_numero: mh,
      categorie_logement: "MH Classique 3 chambres",
      contexte: "ARRIVEE",
      urgent: hasUrgent,
  ...
  const handleFinalSubmit = async () => {
    ...
    try {
      ...
      await createInterventionClassique3ch({ service: "MENAGE", items: menage, ficheId: fiche.id });
      await createInterventionClassique3ch({ service: "TECHNIQUE", items: technique, ficheId: fiche.id });
  ...
  return (
    <div className="min-h-screen max-w-4xl mx-auto px-6 py-8">
      ...
      <p className="text-center text-gray-600 mb-6">
        MH Classique 3ch {mh} • {nom} {prenom}
      </p>

      {Object.entries(INVENTAIRE_CLASSIQUE_3CH).map(([section, items]) => (
            ...
            <h3 className="font-semibold text-[#0077A8] mb-3 capitalize">
              {lang === "fr" 
                ? section === 'vaisselle' ? '🍽️ Vaisselle' 
                  ...
                  : section === 'menage' ? '🧹 Entretien & Nettoyage'
