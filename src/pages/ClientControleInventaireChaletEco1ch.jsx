...
/* ============================================================
   CHALET ÉCO 1CH - INVENTAIRE COMPLET (C01 à C06)
============================================================ */

const INVENTAIRE_CHALET_ECO_1CH = {
  vaisselle: [
    ...
    { id: 'verres', emoji: '🥛', label: 'Verres', qty: 6, service: 'MENAGE' },
  ...
  chambres: [
    { id: 'couettes_doubles', emoji: '🛏️', label: 'Couettes doubles', qty: 2, service: 'MENAGE' },
    { id: 'couette_simple', emoji: '🛏️', label: 'Couette simple', qty: 1, service: 'MENAGE' },
...
export default function ClientControleInventaireChaletEco1ch() {
  ...
  const allItems = useMemo(() => {
    return [
      ...INVENTAIRE_CHALET_ECO_1CH.vaisselle,
      ...INVENTAIRE_CHALET_ECO_1CH.couverts,
      ...INVENTAIRE_CHALET_ECO_1CH.ustensiles,
      ...INVENTAIRE_CHALET_ECO_1CH.electromenager,
      ...INVENTAIRE_CHALET_ECO_1CH.menage,
      ...INVENTAIRE_CHALET_ECO_1CH.chambres,
      ...INVENTAIRE_CHALET_ECO_1CH.exterieur,
      ...INVENTAIRE_CHALET_ECO_1CH.sanitaires
    ];
  }, []);
  ...
  const createInterventionChaletEco = async ({ service, items, ficheId }) => {
    ...
    const intervention = await base44.entities.Intervention.create({
      ...
      logement_numero: mh,
      categorie_logement: "Chalet Éco 1 chambre",
      contexte: "ARRIVEE",
      urgent: hasUrgent,
  ...
  const handleFinalSubmit = async () => {
    ...
    try {
      ...
      await createInterventionChaletEco({ service: "MENAGE", items: menage, ficheId: fiche.id });
      await createInterventionChaletEco({ service: "TECHNIQUE", items: technique, ficheId: fiche.id });
      ...
      <div className="min-h-screen flex items-center justify-center px-4">
        <div className="text-center max-w-md space-y-6">
          ...
          <h2 className="text-2xl font-bold text-[#0077A8]">
            {lang === "fr" 
              ? `Nous vous remercions pour le retour concernant votre hébergement. Nous espérons que vous passerez un excellent séjour parmi nous ${prenom} ${nom}.`
              : `Thank you for your feedback. We hope you have an excellent stay with us ${prenom} ${nom}.`
            }
          </h2>
  ...
  return (
    <div className="min-h-screen max-w-4xl mx-auto px-6 py-8">
      ...
      <p className="text-center text-gray-600 mb-6">
        Chalet Éco 1ch {mh} • {nom} {prenom}
      </p>

      {Object.entries(INVENTAIRE_CHALET_ECO_1CH).map(([section, items]) => (
        <Card key={section} className="mb-6">
          <CardContent className="p-4">
            <h3 className="font-semibold text-[#0077A8] mb-3 capitalize">
              {lang === "fr" 
                ? section === 'vaisselle' ? '🍽️ Cuisine / Vaisselle' 
                  : section === 'couverts' ? '🍴 Couverts'
                  : section === 'ustensiles' ? '🍳 Ustensiles & Cuisson'
                  : section === 'electromenager' ? '🔌 Électroménager'
                  : section === 'menage' ? '🧹 Ménage & Sécurité'
                  : section === 'chambres' ? '🛏️ Chambres'
      ...
      <Button onClick={handlePrepareSubmit} className="w-full h-14 bg-[#00AEEF] mt-6" disabled={submitting}>
        <Send className="mr-2" />
        {lang === "fr" ? "Valider le contrôle" : "Confirm"}
      </Button>
