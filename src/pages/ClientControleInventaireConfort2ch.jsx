...
/* ============================================================
   MH CONFORT 2CH - INVENTAIRE COMPLET (B01 à B12)
============================================================ */

const INVENTAIRE_CONFORT_2CH = {
  vaisselle: [
...
export default function ClientControleInventaireConfort2ch() {
  ...
  const allItems = useMemo(() => {
    return [
      ...INVENTAIRE_CONFORT_2CH.vaisselle,
  ...
  const createInterventionConfort2ch = async ({ service, items, ficheId }) => {
  ...
  const handleFinalSubmit = async () => {
    ...
    try {
      ...
      await createInterventionConfort2ch({ service: "MENAGE", items: menage, ficheId: fiche.id });
      await createInterventionConfort2ch({ service: "TECHNIQUE", items: technique, ficheId: fiche.id });
  ...
  return (
    <div className="min-h-screen max-w-4xl mx-auto px-6 py-8">
      ...
      {Object.entries(INVENTAIRE_CONFORT_2CH).map(([section, items]) => (
