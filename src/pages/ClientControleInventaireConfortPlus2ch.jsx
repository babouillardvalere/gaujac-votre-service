...
/* ============================================================
   MH CONFORT PLUS 2CH - INVENTAIRE COMPLET (P01 à P12)
============================================================ */

const INVENTAIRE_CONFORT_PLUS_2CH = {
  vaisselle: [
    ...
    { id: 'plat', emoji: '🍲', label: 'Plats', qty: 1, service: 'MENAGE' },
  ...
  couverts: [
    ...
    { id: 'couverts_salade', emoji: '🥗', label: 'Couvert à salade', qty: 1, service: 'MENAGE' },
  ...
  batterie: [
    ...
    { id: 'poeles', emoji: '🍳', label: 'Poêle', qty: 2, service: 'MENAGE' },
  ...
  menage: [
    ...
    { id: 'poubelles', emoji: '🗑️', label: 'Poubelle', qty: 2, service: 'MENAGE' },
  ...
  literie: [
    ...
    { id: 'couettes_simples', emoji: '🛏️', label: 'Couette simple', qty: 2, service: 'MENAGE' },
...
export default function ClientControleInventaireConfortPlus2ch() {
  ...
  const allItems = useMemo(() => {
    return [
      ...INVENTAIRE_CONFORT_PLUS_2CH.vaisselle,
  ...
  const createInterventionConfortPlus2ch = async ({ service, items, ficheId }) => {
    ...
    const intervention = await base44.entities.Intervention.create({
      ...
      logement_numero: mh,
      categorie_logement: "MH Confort+ 2 chambres",
      contexte: "ARRIVEE",
      urgent: hasUrgent,
  ...
  const handleFinalSubmit = async () => {
    ...
    try {
      ...
      await createInterventionConfortPlus2ch({ service: "MENAGE", items: menage, ficheId: fiche.id });
      await createInterventionConfortPlus2ch({ service: "TECHNIQUE", items: technique, ficheId: fiche.id });
  ...
  return (
    <div className="min-h-screen max-w-4xl mx-auto px-6 py-8">
      ...
      <p className="text-center text-gray-600 mb-6">
        MH Confort+ 2ch {mh} • {nom} {prenom}
      </p>

      {Object.entries(INVENTAIRE_CONFORT_PLUS_2CH).map(([section, items]) => (
            ...
            <h3 className="font-semibold text-[#0077A8] mb-3 capitalize">
              {lang === "fr" 
                ? section === 'vaisselle' ? '🍽️ Vaisselle' 
                  ...
                  : section === 'appareils' ? '🔌 Équipements / Électroménager'
                  : section === 'sanitaires' ? '🚿 Sanitaires'
                  : section === 'menage' ? '🧹 Entretien & Sécurité'
                  : section === 'literie' ? '🛏️ Literie & Rangement'
                  : section === 'exterieur' ? '🌳 Accès & Mobilier'
                  : section
                : section
              }
            </h3>
