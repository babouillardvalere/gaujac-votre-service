...
/* ============================================================
   MH PREMIUM 3CH - INVENTAIRE COMPLET (M01 à M14)
============================================================ */

const INVENTAIRE_PREMIUM_3CH = {
  vaisselle: [
...
export default function ClientControleInventairePremium3ch() {
  ...
  const allItems = useMemo(() => {
    return [
      ...INVENTAIRE_PREMIUM_3CH.vaisselle,
  ...
  const createInterventionPremium3ch = async ({ service, items, ficheId }) => {
    ...
    const intervention = await base44.entities.Intervention.create({
      ...
      logement_numero: mh,
      categorie_logement: "MH Premium 3 chambres",
      contexte: "ARRIVEE",
      urgent: hasUrgent,
  ...
  const handleFinalSubmit = async () => {
    ...
    try {
      ...
      await createInterventionPremium3ch({ service: "MENAGE", items: menage, ficheId: fiche.id });
      await createInterventionPremium3ch({ service: "TECHNIQUE", items: technique, ficheId: fiche.id });
  ...
  return (
    <div className="min-h-screen max-w-4xl mx-auto px-6 py-8">
      ...
      <p className="text-center text-gray-600 mb-6">
        MH Premium 3ch {mh} • {nom} {prenom}
      </p>

      {Object.entries(INVENTAIRE_PREMIUM_3CH).map(([section, items]) => (
