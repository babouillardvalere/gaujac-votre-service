...
export default function Technique() {
  ...
  const pushClientEvent = async ({ incident, type, message, meta = {} }) => {
    ...
    try {
      ...
      // non-bloquant
      console.error('InterventionEvent create failed:', e);
    }
  };
  ...
  const handlePhotoAvantUploaded = async (photoData) => {
    ...
    // ✅ Event client (photo avant)
    await pushClientEvent({
      incident: incidentForPhoto,
      type: 'PHOTO_AVANT',
      message: "Début d’intervention – état initial constaté.",
      meta: { photo_url: photoData.url }
    });

    // ✅ Event client (prise en charge)
    await pushClientEvent({
      incident: incidentForPhoto,
      type: 'PRISE_EN_CHARGE',
      message: "L’intervention technique a commencé."
  ...
  const handleCopyAvisLink = (incident) => {
    ...
    // ✅ FIX template string (cassait le parsing)
  ...
  const confirmMettreEnAttente = async (formData) => {
    ...
    await pushClientEvent({
      ...
      meta: {
        motif: formData.motifAttente,
        raison: formData.raison,
        materiel: formData.materiel,
        detail: formData.materielDetail,
          ...
          <TabsContent value="interventions">
            ...
            <Tabs value={filter} onValueChange={setFilter} className="mb-6 mt-6">
              <TabsList className="bg-[#e6f7ff] p-1 rounded-xl border border-[#00AEEF]/30 w-full grid grid-cols-4">
                ...
                <TabsTrigger
                  value="en_cours"
                  className="rounded-lg font-heading text-xs data-[state=active]:bg-[#00AEEF] data-[state=active]:text-white relative"
                >
                  {t('en_cours')}
                  <span className="ml-1 min-w-5 h-5 inline-flex items-center justify-center text-xs font-bold rounded-full px-1.5 bg-[#E63946] text-white">
                ...
                {filteredIncidents.map((incident) => {
                  ...
                  return (
                    <motion.div key={incident.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
                      {/* ✅ FIX className template literal */}
                      <Card
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                <div className="flex items-center gap-2 flex-wrap">
                                  ...
                                  {priorityType === 'urgent' && (
                                    <Badge className="bg-red-500 text-white text-xs">⚠️ {t('urgent_label')}</Badge>
                                  )}
                          ...
                          <div className="flex items-center justify-between text-xs text-gray-500 font-body">
                            ...
                            <div className="flex items-center gap-1">
                              ...
                              {incident.date_saisie &&
                                format(new Date(incident.date_saisie), 'dd/MM HH:mm', { locale: fr })}
                            </div>
                          </div>
                              ...
                              {incident.autorisation_acces === 'oui' ? (
                                ...
                                <>
                                  ...
                                  {incident.plage_horaire_client && (
                                    <span className="ml-1">
                                      — {t('plage_demandee')}: {incident.plage_horaire_client}
                                    </span>
                                  )}
                                </>
                              )}
                          ...
                          {incident.pris_par && (
                            <div className="mt-2 pt-2 border-t border-gray-100 flex items-center justify-between">
                              <p className="text-xs font-body text-[#00AEEF]">
                                {t('pris_en_charge_par')}: {incident.pris_par}
                              </p>
                ...
                return filteredTaches.length === 0 ? (
                  <div className="text-center py-12">
                    ...
                    <p className="font-heading text-[#0077A8]">
                      {lang === 'fr' ? 'Aucune tâche trouvée' : 'No tasks found'}
                    </p>
                  </div>
                ) : (
                    ...
                    <Card key={tache.id} className="border-2 border-[#00AEEF]/30 rounded-xl">
                      <CardContent className="p-4">
                        ...
                        <div className="flex flex-wrap gap-2 mt-2">
                          ...
                          {tache.hebergement && <Badge variant="outline">🏠 {tache.hebergement}</Badge>}
                          {tache.date_echeance && (
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          {selectedIncident && (
            <div className="space-y-4">
                      ...
                      <span className="font-body text-sm">
                        ...
                        {selectedIncident.plage_horaire_client && (
                          <span className="block text-xs mt-1">
                            📅 {t('plage_demandee')}: {selectedIncident.plage_horaire_client}
                          </span>
                        )}
                      </span>
                    </>
              ...
              <InterventionDocuments incidentId={selectedIncident.id} documents={documents} canAdd={true} />
              ...
              {selectedIncident.statut === 'en_attente_materiel' && (
                <div className="space-y-3 pt-4 border-t">
                  <div className="bg-[#FFA500]/10 p-4 rounded-xl border border-[#FFA500]/30">
                    ...
                    {selectedIncident.attente_delai && (
                      <p className="font-body text-gray-500 text-xs mt-1">
                        {t('delai_estime')} : {selectedIncident.attente_delai}
                      </p>
                    )}
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <Button
                      onClick={() => handleReprendre(selectedIncident)}
                  ...
                  <div className="bg-green-50 p-3 rounded-xl">
                    <p className="text-sm font-body text-green-700">
                      ...
                      {selectedIncident.date_resolution &&
                        format(new Date(selectedIncident.date_resolution), 'dd/MM/yyyy HH:mm')}
                    </p>
                  </div>
