...
export default function ClientControleInventaireClassiqueClim2ch() {
  ...
  const allItems = useMemo(() => {
    return [
      ...INVENTAIRE_CLASSIQUE_CLIM_2CH.vaisselle,
  ...
  const createInterventionClassiqueClim2ch = async ({ service, items, ficheId }) => {
    ...
    const intervention = await base44.entities.Intervention.create({
      ...
      logement_numero: mh,
      categorie_logement: "MH Classique Clim 2 chambres",
      contexte: "ARRIVEE",
      urgent: hasUrgent,
  ...
  const handleFinalSubmit = async () => {
    ...
    try {
      ...
      await createInterventionClassiqueClim2ch({ service: "MENAGE", items: menage, ficheId: fiche.id });
      await createInterventionClassiqueClim2ch({ service: "TECHNIQUE", items: technique, ficheId: fiche.id });
  ...
  return (
    <div className="min-h-screen max-w-4xl mx-auto px-6 py-8">
      ...
      <p className="text-center text-gray-600 mb-6">
        MH Classique Clim 2ch {mh} • {nom} {prenom}
      </p>

      {Object.entries(INVENTAIRE_CLASSIQUE_CLIM_2CH).map(([section, items]) => (
