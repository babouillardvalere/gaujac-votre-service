...
/* ============================================================
   MH PREMIUM TWINS - INVENTAIRE COMPLET (R11/R04/R12/R05)
============================================================ */
...
export default function ClientControleInventairePremiumTwins() {
  ...
  const allItems = useMemo(() => {
    return [
      ...INVENTAIRE_PREMIUM_TWINS.vaisselle,
  ...
  const createInterventionPremiumTwins = async ({ service, items, ficheId }) => {
    ...
    const intervention = await base44.entities.Intervention.create({
      ...
      logement_numero: mh,
      categorie_logement: "MH Premium Twins",
      contexte: "ARRIVEE",
      urgent: hasUrgent,
  ...
  const handleFinalSubmit = async () => {
    ...
    try {
      ...
      await createInterventionPremiumTwins({ service: "MENAGE", items: menage, ficheId: fiche.id });
      await createInterventionPremiumTwins({ service: "TECHNIQUE", items: technique, ficheId: fiche.id });
  ...
  return (
    <div className="min-h-screen max-w-4xl mx-auto px-6 py-8">
      ...
      <p className="text-center text-gray-600 mb-6">
        MH Premium Twins {mh} • {nom} {prenom}
      </p>

      {Object.entries(INVENTAIRE_PREMIUM_TWINS).map(([section, items]) => (
