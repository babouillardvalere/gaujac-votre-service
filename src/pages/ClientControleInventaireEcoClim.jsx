...
/* ============================================================
   MH ECO CLIM - INVENTAIRE COMPLET (O01 à O05)
============================================================ */

const INVENTAIRE_ECO_CLIM = {
  vaisselle: [
...
export default function ClientControleInventaireEcoClim() {
  ...
  const allItems = useMemo(() => {
    return [
      ...INVENTAIRE_ECO_CLIM.vaisselle,
      ...INVENTAIRE_ECO_CLIM.couverts,
      ...INVENTAIRE_ECO_CLIM.batterie,
      ...INVENTAIRE_ECO_CLIM.appareils,
      ...INVENTAIRE_ECO_CLIM.sanitaires,
      ...INVENTAIRE_ECO_CLIM.menage,
      ...INVENTAIRE_ECO_CLIM.literie,
      ...INVENTAIRE_ECO_CLIM.exterieur
    ];
  }, []);
  ...
  const createInterventionEcoClim = async ({ service, items, ficheId }) => {
    ...
    const intervention = await base44.entities.Intervention.create({
      ...
      logement_numero: mh,
      categorie_logement: "MH Éco Clim",
      contexte: "ARRIVEE",
      urgent: hasUrgent,
  ...
  const handleFinalSubmit = async () => {
    ...
    try {
      ...
      await createInterventionEcoClim({ service: "MENAGE", items: menage, ficheId: fiche.id });
      await createInterventionEcoClim({ service: "TECHNIQUE", items: technique, ficheId: fiche.id });
  ...
  return (
    <div className="min-h-screen max-w-4xl mx-auto px-6 py-8">
      ...
      <p className="text-center text-gray-600 mb-6">
        MH Éco Clim {mh} • {nom} {prenom}
      </p>

      {Object.entries(INVENTAIRE_ECO_CLIM).map(([section, items]) => (
